...
import {
  SortPostsDropdown,
  SortOption,
} from "../../components/SortPostsDropdown"; // Corrected path
...
async function getUserProfileData(
  userName: string,
  searchParamsPropFromPage: Promise<ResolvedSearchParams>
) {
  // searchParamsPropFromPage is a Promise
  noStore();
  ...
  if (
    ...
    typeof resolvedSearchParams.postsPage === "string"
  ) {
  ...
  if (
    resolvedSearchParams.commentsPage &&
    typeof resolvedSearchParams.commentsPage === "string"
  ) {
  ...
  const postsSortToUse =
    resolvedSearchParams.postsSort &&
    validSortOptions.includes(resolvedSearchParams.postsSort as SortOption)
  ...
  const posts = await prisma.post.findMany({
    ...
    select: {
      // Use select exclusively, nest relations
      id: true,
      title: true,
      textContent: true,
      imageString: true,
  ...
  userPostsForActivity.forEach((post) => {
    if (post.subName) {
      const hubName = post.subName.startsWith("bhub/")
        ? post.subName.substring(5)
        : post.subName;
      if (!bhubActivity[hubName]) {
        bhubActivity[hubName] = {
          name: hubName,
          posts: 0,
          comments: 0,
          score: 0,
  ...
  userCommentsForActivity.forEach((comment) => {
    if (comment.Post?.subName) {
      const hubName = comment.Post.subName.startsWith("bhub/")
        ? comment.Post.subName.substring(5)
...
export default async function UserProfilePage({
  ...
  const profileDataBundle = await getUserProfileData(
    username,
    searchParamsProp
  );
      ...
      <Card>
        ...
        <CardContent className="p-6 space-y-6">
          ...
          <div className="space-y-4">
            {user.userPrimaryRole && (
              <div className="flex items-start">
                ...
                <div>
                  ...
                  <p className="text-sm">
                    {user.userPrimaryRole.charAt(0).toUpperCase() +
                      user.userPrimaryRole.slice(1).toLowerCase()}
                  </p>
                </div>
              </div>
            )}
            ...
            {(user.userPrimaryRole === "STUDENT" ||
              user.userPrimaryRole === "BOTH") &&
              (user.nim || user.studentMajor || user.studentBatch) && (
            ...
            {(user.userPrimaryRole === "EMPLOYEE" ||
              user.userPrimaryRole === "BOTH") &&
              (user.employeeId || user.employeeDepartment) && (
                <div className="flex items-start mt-4">
      ...
      <Tabs defaultValue={determinedActiveTab} className="w-full">
        <TabsList className="grid w-full grid-cols-3">
          <TabsTrigger value="posts" asChild>
            <Link href={`/profile/${username}?tab=posts`} scroll={false}>
              Posts ({postsCount})
            </Link>
          </TabsTrigger>
          <TabsTrigger value="comments" asChild>
            <Link href={`/profile/${username}?tab=comments`} scroll={false}>
        ...
        <TabsContent value="posts" className="mt-2 space-y-4">
          ...
          {posts.length > 0 ? (
            posts.map((post) => {
              ...
              const currentUserVoteOnPost =
                viewingUser?.id && Array.isArray(post.vote)
                  ? post.vote.find((v) => v.userId === viewingUser.id)
                      ?.voteType ?? null
                  : null;
        ...
        <TabsContent value="comments" className="mt-6 space-y-4">
            ...
            comments.map((comment) => (
              <Card key={comment.id} className="p-4">
                ...
                {comment.Post && (
                  ...
                  >
                    View post: "{comment.Post.title}" in bhub/
                    {comment.Post.subName || "General"}
                  </Link>
                )}
              </Card>
            ))
          ...
          {commentsCount > COMMENTS_PER_PAGE && (
            <Pagination
              totalPages={Math.ceil(commentsCount / COMMENTS_PER_PAGE)}
            />
          )}
        </TabsContent>
        ...
        <TabsContent value="activity" className="mt-6">
          {topBHubs && topBHubs.length > 0 ? (
              ...
              <CardContent className="p-0">
                <ul className="divide-y divide-border">
                  {topBHubs.map((hub) => (
                    <li
                      key={hub.name}
                      className="flex flex-col sm:flex-row justify-between sm:items-center p-4 hover:bg-muted/50 transition-colors"
                    >
                      <Link
                        href={`/p/${hub.name}`}
                        scroll={false}
                        className="text-primary hover:underline font-semibold text-md mb-1 sm:mb-0"
                      >
                        bhub/{hub.name}
                      </Link>
                      <div className="flex items-center space-x-2 mt-1 sm:mt-0">
                        <Badge variant="outline">
                          {hub.posts} {hub.posts === 1 ? "post" : "posts"}
                        </Badge>
                        <Badge variant="outline">
                          {hub.comments}{" "}
                          {hub.comments === 1 ? "comment" : "comments"}
                        </Badge>
                        <Badge variant="secondary">
                          {hub.score} total activity
                        </Badge>
                      </div>
                    </li>
                  ))}
            ...
            <Card className="flex items-center justify-center h-40">
              <p className="text-muted-foreground">
                This user has no significant BHub activity yet.
              </p>
            </Card>
          )}
        </TabsContent>
